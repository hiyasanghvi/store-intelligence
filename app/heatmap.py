...
def get_store_heatmap(store_id: str, db: Session) -> HeatmapResponse:
    ...
    for row in zone_rows:
        ...
        if row.zone_id in [
            ...
            continue
        dwell_seconds = round((row.avg_dwell_ms or 0) / 1000, 1)

        curiosity_score = round(
            row.visitor_count * dwell_seconds,
            1
        )

        if curiosity_score >= 80:
            engagement_band = "HOT"
        elif curiosity_score >= 30:
        ...
        zones.append(
            HeatmapZone(
                ...
                normalised_score=round(
                    (row.visitor_count / max_count) * 100,
                    1
    ...
    zones.sort(
        key=lambda z: z.curiosity_score,
        reverse=True
    )

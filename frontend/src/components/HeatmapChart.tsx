...
export const HeatmapChart: React.FC<HeatmapChartProps> = ({ zones }) => {
  ...
  console.log("ZONE COUNT", sorted.length);
  return (
    <div className="heatmap-grid-cells">
      {sorted.map((zone) => {
        console.log("HEATMAP ZONE", zone);
  ...
  <span>{zone.engagement_band || `${zone.visit_count} visits`}</span>
  <span>{zone.sku_zone || 'avg dwell'}</span>
</div>

<div className="heatmap-curiosity">
   Curiosity Score: {zone.curiosity_score?.toFixed(1) ?? 0}
</div>
          </div>
        );
      })}
